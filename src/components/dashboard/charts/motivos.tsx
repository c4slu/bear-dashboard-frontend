...
export default function RoscaDash() {
  const [dados, setDados] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Axios.get("http://localhost:3001/motivos").then((response) => {
      setDados(response.data);
      setLoading(false);
    });
  }, []);

  const colunas = dados.map((item: any) => item.motivo);
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center ">
      <p className="text-xs text-muted-foreground font-semibold">Motivos</p>
